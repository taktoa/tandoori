...
module Tandoori.Typing.Show (printCtx, showName) where
...
data ShowTyCtx = C { isLeftOfFun :: Bool, isRightOfApp :: Bool }

showFunLeft :: ShowTyCtx -> Ty -> String
...
prettyTypingErrorM (UnfulfilledPredicate (cls, τ)) = UnfulfilledPredicate . (cls,) <$> prettyTyM τ
...
instance Show MonoEnv where
  show m = "{" ++ intercalate ", " (typing ++ preds) ++ "}"
    where
      typing = map (\ (x, τ) -> showName x ++ "::" ++ show τ) $ getMonoVars m
...
printCtx :: Context -> IO ()
printCtx c = Box.printBox $ boxName Box.<+> boxType
  where
    showPolyTy = show . runPretty . prettyPolyTyM
...
boxMonos ms = Box.hsep 2 Box.top $ boxNames:(map boxTypes ms)
  where
    ...
    boxNames = Box.vcat Box.left $ (Box.text ""):
                                   (Box.text ""):
                                   (Box.text "Predicates:"):
                                   (map ((Box.<+> Box.text "::") . (Box.text . showName)) vars)
    boxTypes m = Box.vcat Box.left $ boxSrc:boxTy:boxPreds:boxsTyVars
      where
        dflags = defaultDynFlags (Settings {})
        boxSrc = Box.text $ maybe "" (showSDocUnqual dflags) $ getMonoSrc m
